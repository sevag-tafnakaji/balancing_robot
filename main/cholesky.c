...
int cholesky(float** orig, int n, float** aug, int mcol, float** chol,
             float** cholaug, int ofs)
/*
   ...
   orig (input) float n x n array. The matrix to take the Cholesky
         decomposition of.
   n    (input) integer. Number of rows and columns in orig.
   aug  (input) float n x mcol array. The matrix for the augmented
         part of the decomposition.
   mcol (input) integer. Number of columns in aug.
   chol (output) float n x n array. Holds the upper triangular matrix
   ...
   cholaug (output) float n x mcol array. Holds the product C^-t * aug.
...
*/
{
  ...
  for (i = ofs; i < n + ofs; i++) {
    chol[i][i] = orig[i][i];
    for (k = ofs; k < i; k++) chol[i][i] -= chol[k][i] * chol[k][i];
    if (chol[i][i] <= 0) {
    ...
    for (j = i + 1; j < n + ofs; j++) {
      chol[i][j] = orig[i][j];
      for (k = ofs; k < i; k++) chol[i][j] -= chol[k][i] * chol[k][j];
      chol[i][j] /= chol[i][i];
    }
  }
